...
namespace RandomFromDistributions.Examples.DistributionsExamples {
  public class CustomPDFTestScript : MonoBehaviour {
    ...
    void Start() { this.graph = this.CreateGraph(); }
    ...
    private GameObject CreateGraph() {
      var buckets =
          new int[this.distribution.Length]; // add one, because RandomRangeNormalDistribution is inclusive.
      for (var i = 0; i < buckets.Length; ++i) {
        buckets[i] = 0;
      }

      for (var i = 0; i < this.repetitions; ++i) {
        var bucket = this.GetRandomNumber();
